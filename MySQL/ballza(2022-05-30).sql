...
-- 가장 먼저 어떤 DB 를 사용할 것인지 지정해야 한다

use mydb;
drop table tbl_test;
-- table 을 생성할때 칼럼의 이름은 가급적 사전에 있는 키워드를 사용하지 않는것이 좋다
-- 사전에 있는 키워드를 사용할시 접두사를 붙여 사용하는것이 안전하다
...
MySQL 도 널리 사용된다
MySQL 에서 사용하는 방식이 DBMS 의 업게 표준으로 
정착이 된 상태

대부분의 DBMS 들은 먼저 Database 를 생성하고
사용자가 Database 에 접근할 수 있는 권한을 부여한다
사용자는 Database 를 사용하겠다고 선언 한 후
DB 관련된 명령을 사용한다
*/
...
때문에 정수형 11자리 범위를 벗어나는 경우 데이터 insert가 불가능했다
5.7 버전 이후에는 bigint 형 타입이 도입되어 문제를 해결하였다
DB table 을 만들때 auto_increment 칼럼에는 반드시 bigint 형을 지정한다

mySQL 숫자형 데이터 타입
tinyint : 1Byte : -128~ 127 까지 저장가능
smallint : 2 Byte : -32768 ~ 23762 까지
mediumint : 3Byte : 0 ~ 16777215, -8388608 ~ 8388607 까지
...
데이터 타입은 bigint로 설정
가급적 primary key 로 설정
pk 아닌 경우는 not null 과 unique 를 같이 설정한다
...
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0102','이몽룡','법학',2,'010-1111-1112','익산시');
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0101','고길동','컴공과',3,'010-1111-1111','익산시');
