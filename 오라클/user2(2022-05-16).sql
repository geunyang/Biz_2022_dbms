...
SEQ 칼럼이 없이 PK 를 설정하려면 학번 + 과목명 으로 두개의 칼럼을 묶어서
PK로 해야한다
다수의 칼럼을 묶음으로 PK를 설정하는 것을 Super Key  라고 부른다
필요에 따라 Super key 로 PK 를 설정 하기도 하지만 SuperKey 는
잘못 사용하면 데이터 무결성을 해칠수 있다
...
-- 학번 S0044인 학생의 미술점수를 50 점에서 80 점으로 바꾸고 싶다
-- Update Delete 실행할때 조건절(WHERE) 에 복수의 칼럼 조건을 부여하는 것은 좋지 않다
...
-- 성적 조회를 하였는데 학번과 점수만 나타나 이름을 알수가 없다
